...
invoke() {
  ...
  local response
  # Pipe body via stdin instead of -d "$body": on Windows (Git Bash/MSYS2),
  # passing non-ASCII argv to native curl re-encodes it to the system codepage
  # (e.g. GBK), which AnkiConnect then fails to decode as UTF-8.
  response=$(printf '%s' "$body" | curl -fsS "$ANKI_URL" --data-binary @-) || { echo "curl failed" >&2; exit 1; }
  # Check if response contains "error": null (with space) using grep to avoid UTF-8 issues with jq
  if echo "$response" | grep -q '"error": null'; then
    echo "$response" | jq '.result'
  else
    local error_msg
    error_msg=$(echo "$response" | grep -o '"error": "[^"]*"' | cut -d'"' -f4 | head -1)
...
case "$action" in
  ...
  tags)     cmd_tags "$@" ;;
  *)
    echo "Usage: anki.sh <action> [args...]" >&2
    echo "Actions: sync, decks, models, fields, find, info, add, add-bulk, update, delete, due, review, rate, tags" >&2
    exit 1
    ;;
esac
